# Rollback Netcode: Insights & Setbacks
...
This seemingly simple approach fails for two reasons:
...
* **The Problem of "Now":** Due to network latency, data doesn't arrive instantly. Player A's state from a specific moment would arrive at Player B's machine a fraction of a second later. In that time, Player B's own state has already changed. Trying to reconcile these constantly arriving, out-of-date "snapshots" would result in a chaotic, jittery mess rather than a synchronized game. There is no single, shared "now."
...
![Flowchart of the rollback process](/images/blogs/rollbacknetcode/RollbackChart.png "Title")
